...
- [ ] **Do:** Encode the 15 kettle fields (tier, value kind, question text, rationale, declaration order) and the dossier/candidate/citation types + Zod extraction schema. Schema requires document, page, and quote on every candidate. No confidence field.
- **Blocked by:** T1
- **Plan mode:** high
- **TDD suitable:** yes
...
- [ ] **Do:** Expose `POST /api/extract` and `POST /api/interpret`. Reject bad type/size before parse. Missing-key error names `GEMINI_KEY` and recorded mode. Partial multi-document failure retains successful facts.
- **Blocked by:** T6
- **Plan mode:** medium
- **TDD suitable:** yes
